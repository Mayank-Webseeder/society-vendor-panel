...
const OngoingJobs = () => {
  ...
  const JobCard = ({ job, index }) => (
    ...
    >
      ...
      <div className="relative p-3 sm:p-5">
        ...
        <div className="flex items-start justify-between mb-3 sm:mb-4">
          <div className="flex items-center gap-2 sm:gap-3 flex-1">
            <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-br from-orange-400/20 via-orange-400/10 to-transparent border border-solid border-orange-300/40 flex items-center justify-center group-hover:from-orange-400/30 group-hover:via-orange-400/20 transition-colors backdrop-blur-sm">
              <Wrench className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-orange-600" />
            </div>
            <div className="flex-1">
              <h4 className="font-semibold text-slate-800 text-sm sm:text-base mb-1 group-hover:text-orange-700 transition-colors line-clamp-1">
                {job.work}
              </h4>
              <div className="h-0.5 w-10 bg-gradient-to-r from-orange-500/60 via-amber-400/60 to-transparent rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            </div>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2">
            <span className="text-[10px] sm:text-xs font-semibold tracking-wide text-orange-700 bg-orange-50/70 backdrop-blur border border-solid border-orange-200/70 px-2 sm:px-2.5 py-1 rounded-lg shadow-sm">
              Ongoing
            </span>
            <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5 text-slate-400 group-hover:text-orange-500 transform group-hover:translate-x-1 transition-all duration-200" />
          </div>
        </div>

        {/* Property Name */}
        <div className="mb-2 sm:mb-3">
          <h5 className="font-medium text-slate-900 text-xs sm:text-sm">{job.name}</h5>
        </div>

        {/* Address */}
        <div className="flex items-start gap-2 sm:gap-2.5 mb-3 sm:mb-4">
          <div className="w-6 h-6 sm:w-7 sm:h-7 bg-orange-50/60 rounded-md flex items-center justify-center mt-0.5 flex-shrink-0 border border-solid border-orange-100/70">
        ...
        <div className="flex items-center gap-3 sm:gap-4 text-[11px] sm:text-xs font-medium">
          <div className="flex items-center gap-1 sm:gap-1.5 text-slate-600">
            <div className="w-5 h-5 sm:w-6 sm:h-6 bg-blue-50/70 rounded-md flex items-center justify-center border border-solid border-blue-100/70">
          ...
          {job.dueTime && (
            <div className="flex items-center gap-1 sm:gap-1.5 text-red-600">
              <div className="w-5 h-5 sm:w-6 sm:h-6 bg-red-50/70 rounded-md flex items-center justify-center border border-solid border-red-100/70">
      ...
      {/* Bottom accent */}
      <div className="h-1 bg-gradient-to-r from-orange-500 via-amber-400 to-orange-600 opacity-0 group-hover:opacity-100 transition-all duration-500" />
    </motion.div>
  );
          ...
          {topJobs.length === 0 || !subscriptionActive ? (
            <div className="col-span-2 relative rounded-2xl border border-solid border-slate-200/70 bg-white/60 backdrop-blur-sm py-10 sm:py-14 text-center shadow-inner">
              <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 h-px w-40 bg-gradient-to-r from-transparent via-orange-400/50 to-transparent" />
              </div>
              ...
              <p className="text-slate-600 text-sm sm:text-base font-semibold mb-1">No ongoing jobs</p>
              <p className="text-slate-400 text-xs sm:text-sm">Currently ongoing jobs will appear here</p>
              {/* <div className="mt-6 h-1 w-28 mx-auto bg-gradient-to-r from-orange-400/50 via-amber-400/40 to-transparent rounded-full" /> */}
            </div>
          ) : (
