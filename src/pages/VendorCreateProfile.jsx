...
export default function VendorCreateProfile() {
    ...
    useEffect(() => {
        const loadServices = async () => {
            try {
                ...
                const mockServices = [
                    {
                        ...
                        name: 'Plumber',
                        categories: [
                            { _id: 'c1', name: 'Pipe Repair' },
                            { _id: 'c2', name: 'Leak Fixing' },
                            { _id: 'c3', name: 'Installation' }
                        ]
                    },
                    {
                        _id: '2',
                        ...
                        categories: [
                            { _id: 'c4', name: 'Wiring' },
                            { _id: 'c5', name: 'Appliance Repair' },
                            { _id: 'c6', name: 'Panel Upgrade' }
                        ]
                    },
                    {
                        _id: '3',
                        name: 'Carpenter',
                        categories: [
                            { _id: 'c7', name: 'Furniture Making' },
                            { _id: 'c8', name: 'Door Repair' },
                            { _id: 'c9', name: 'Custom Woodwork' }
                        ]
                    },
                    {
                        _id: '4',
                        ...
                        categories: [
                            { _id: 'c10', name: 'Home Security' },
                            { _id: 'c11', name: 'Commercial Setup' },
                            { _id: 'c12', name: 'Maintenance' }
                        ]
                    },
                    {
                        _id: '5',
                        name: 'Maid Services',
                        categories: [
                            { _id: 'c13', name: 'House Cleaning' },
                            { _id: 'c14', name: 'Cooking' },
                            { _id: 'c15', name: 'Laundry' }
                        ]
                    }
                ];
                ...
                // const res = await api.get('/api/admin/services/with-categories');
                // setServices(res.data.data || []);
            } catch (err) {
                setErrorText('Failed to load services');
                setLoadingServices(false);
            }
        };
        loadServices();
    ...
    const toggleService = (serviceId) => {
        setForm(prev => {
            const exists = prev.services.find(s => s.service === serviceId);
            return {
                ...prev,
                services: exists
                    ? prev.services.filter(s => s.service !== serviceId)
                    : [...prev.services, { service: serviceId, categories: [] }],
            };
        });
    };

    /* ------------------ CATEGORY TOGGLE ------------------ */
    const toggleCategory = (serviceId, categoryId) => {
        setForm(prev => ({
            ...prev,
            services: prev.services.map(s => {
                if (s.service !== serviceId) return s;
                const exists = s.categories.includes(categoryId);
                return {
                    ...s,
                    categories: exists
    ...
    const getCurrentLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                handleChange('location.formattedAddress', formattedAddress);
                setGettingLocation(false);
                setSuccessText('Location detected successfully!');
        ...
        switch (step) {
            case 1:
                ...
                if (!form.businessName.trim()) {
                    setErrorText('Please enter your business name');
                    return false;
                }
                return true;
                
            case 2:
                if (!form.address.buildingNumber.trim() || !form.address.locality.trim() ||
                    !form.address.city.trim() || !form.address.state.trim() || !form.address.pincode.trim()) {
                    setErrorText('Please fill in all required address fields');
                    return false;
                }
                return true;
            ...
            case 3:
                if (form.services.length === 0) {
                    setErrorText('Please select at least one service');
                    return false;
                }
                const hasCategories = form.services.every(s => s.categories.length > 0);
                if (!hasCategories) {
                    setErrorText('Please select at least one category for each selected service');
                    return false;
                }
                if (!form.experience) {
                    setErrorText('Please select your experience level');
                    return false;
                }
                if (!form.location.GeoLocation.latitude || !form.location.GeoLocation.longitude) {
                    setErrorText('Please detect your location or enter coordinates manually');
                    return false;
                }
                return true;
                
            case 4:
                if (!form.idProof.fileBase64) {
                    setErrorText('Please upload an ID proof document');
                    return false;
                }
                return true;
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-3">
                                Services Offered (Select at least one service and category) *
                            </label>
                            
                            {form.services.length > 0 && (
                                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                                    <p className="text-sm text-blue-800">
                                        <strong>Selected: {form.services.length}</strong> service(s) with{' '}
                                        <strong>{form.services.reduce((acc, s) => acc + s.categories.length, 0)}</strong> category(ies)
                                    </p>
                                </div>
                            )}
                            ...
                            {loadingServices ? (
                                ...
                                <div className="space-y-4">
                                    {services.map(service => {
                                        const selectedService = form.services.find(s => s.service === service._id);
                                        return (
                                            <div key={service._id} className={`border rounded-lg p-4 transition-colors ${
                                                selectedService ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
                                            }`}>
                                                ...
                                                {selectedService && service.categories.length > 0 && (
                                                    <div className="mt-4 ml-8 pl-4 border-l-2 border-blue-300">
                                                        <p className="text-xs font-medium text-gray-600 mb-2">Select Categories (at least one):</p>
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                                            {service.categories.map(cat => (
                                                                <label key={cat._id} className="flex items-center gap-2 cursor-pointer p-2 rounded hover:bg-blue-100">
                                                                    <input
                                                                        type="checkbox"
                                                                        checked={selectedService.categories.includes(cat._id)}
                                                                        onChange={() => toggleCategory(service._id, cat._id)}
                                                                        className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                                                                    />
